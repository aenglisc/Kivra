-module(openapi_sender_server_router).

-export([get_paths/1, get_validator_state/0]).

-type operations() :: #{
    Method :: binary() => openapi_sender_server_api:operation_id()
}.
...
get_operations() ->
    #{ 
        'V1UploadPost' => #{
            path => "/v1/upload",
            method => <<"POST">>,
            handler => 'openapi_sender_server_default_handler'
        }
    }.
...
get_openapi_path() ->
    ...
    filename:join(openapi_sender_server_utils:priv_dir(AppName), "openapi.json").
