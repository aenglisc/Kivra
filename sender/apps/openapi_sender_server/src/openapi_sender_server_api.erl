-module(openapi_sender_server_api).
...
request_param_info('V1UploadPost', 'V1UploadPost') ->
    #{
        source =>   body,
        rules => [
            schema,
...
validate_response('V1UploadPost', 201, Body, ValidatorState) ->
    validate_response_body('_v1_upload_post_201_response', '_v1_upload_post_201_response', Body, ValidatorState);
...
validate(Rule = {type, 'integer'}, Name, Value, _ValidatorState) ->
    try
        {ok, openapi_sender_server_utils:to_int(Value)}
    catch
        error:badarg ->
...
validate(Rule = {type, 'float'}, Name, Value, _ValidatorState) ->
    try
        {ok, openapi_sender_server_utils:to_float(Value)}
    catch
        error:badarg ->
            validation_error(Rule, Name)
    end;
...
validate(Rule = schema, Name, Value, ValidatorState) ->
    Definition =  list_to_binary("#/components/schemas/" ++ openapi_sender_server_utils:to_list(Name)),
    try
...
get_value(qs_val, Name, Req) ->
    ...
    Value = openapi_sender_server_utils:get_opt(openapi_sender_server_utils:to_qs(Name), QS),
    {Value, Req};

get_value(header, Name, Req) ->
    Headers = cowboy_req:headers(Req),
    Value =  maps:get(openapi_sender_server_utils:to_header(Name), Headers, undefined),
    {Value, Req};

get_value(binding, Name, Req) ->
    Value = cowboy_req:binding(openapi_sender_server_utils:to_binding(Name), Req),
    {Value, Req}.
...
binary_to_lower(V) when is_binary(V) ->
    list_to_binary(string:to_lower(openapi_sender_server_utils:to_list(V))).
