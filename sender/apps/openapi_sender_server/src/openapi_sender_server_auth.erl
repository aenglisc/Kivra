-module(openapi_sender_server_auth).
...
authorize_api_key(LogicHandler, OperationID, From, KeyParam, Req0) ->
    ...
    case ApiKey of
        ...
        _ ->
            Result = openapi_sender_server_logic_handler:authorize_api_key(
                LogicHandler,
                OperationID,
                ApiKey
            ),
...
get_api_key(header, KeyParam, Req) ->
    ...
    {
        maps:get(
            openapi_sender_server_utils:to_header(KeyParam),
            Headers,
            undefined
        ),
        Req
...
get_api_key(qs_val, KeyParam, Req) ->
    ...
    { openapi_sender_server_utils:get_opt(KeyParam, QS), Req}.
