%% basic handler
-module(openapi_sender_server_default_handler).
...
-record(state, {
    operation_id :: openapi_sender_server_api:operation_id(),
    logic_handler :: atom(),
...
-spec init(Req :: cowboy_req:req(), Opts :: openapi_sender_server_router:init_opts()) ->
...
handle_request_json(
    ...
    case openapi_sender_server_api:populate_request(OperationID, Req0, ValidatorState) of
        {ok, Populated, Req1} ->
            {Code, Headers, Body} = openapi_sender_server_logic_handler:handle_request(
                LogicHandler,
                OperationID,
                Req1,
                maps:merge(State#state.context, Populated)
            ),
            _ = openapi_sender_server_api:validate_response(
                OperationID,
                Code,
                Body,
                ValidatorState
